...
from Course.forms import LoginForm, CompanyRegistrationForm, UserRegistrationForm, QuestionForm
from Course.models import User, Question, Clas, UserClas
import random

# class QuestionView(ModelView):
#     column_display_pk = True # optional, but I like to see the IDs in the list
#     column_hide_backrefs = False
#     column_list = ('id', 'question_type', 'year', 'semester', 'question_text', 'class_id', 'tests')
#
#
# admin.add_view(QuestionView(Question, db.session))
# admin.add_view(ModelView(Clas, db.session))
# admin.add_view(ModelView(UserClas, db.session))
...
def intro(class_id):
    ...
    for c in course.users:
        if c.user_id == current_user.id:
            enrolled = True
            mark = c.mark
    return render_template('intro.html', c=course, enrolled=enrolled, mark=mark, courses=Clas.query.all())
...
def get_class(class_id):
    ...
    if not enrolled:
        ...
        db.session.commit()
        flash(f'You have been successfully enrolled in {c.description}', 'success')
...
def admin():
    ...
    if form.validate_on_submit():
        ...
        user = User(username=form.username.data, password=hashed_password, role='student', company=current_user.company)
        db.session.add(user)
        db.session.commit()
        return redirect(url_for('admin'))
    return render_template('user_admin.html', title='admin', form=form, users=users)
...
def test_admin():
    if current_user.role != 'admin':
        abort(403)
    tests = Clas.query.all()
    return render_template('test_admin.html', title='admin',tests=tests)
...
def exam():
    ...
    current_year = 2021
    current_semester = 2
    q = Question.query.filter_by(clas_id=class_id, year=current_year, semester=current_semester).all()[:5]
    time_limit = Clas.query.filter_by(id=class_id).first().limit_time
    if typ=='mock':
        q = Question.query.filter_by(clas_id=class_id).filter(Question.year!=current_year and Question.semester!=current_semester).all()
        q = random.choices(q, k=5)
    return render_template('test.html', title='Test', q=q, class_id=class_id, type=typ, time=time_limit)
...
def super_admin():
    ...
    if form.validate_on_submit():
        hashed_password = bcrypt.generate_password_hash(form.password.data).decode('utf8')
        user = User(username=form.username.data, password=hashed_password, role='admin', company=form.username.data)
        db.session.add(user)
        db.session.commit()
...
def question_admin():
    ...
    if form.validate_on_submit():
        q = Question(question_type=form.question_type.data, year=form.year.data, semester=form.semester.data, question_text=form.question_text.data, clas_id=form.course.data)
        db.session.add(q)
        db.session.commit()
        flash('Question Added', 'success')
        return redirect(url_for('question_admin', class_id=clas_id, year=year, semester=semester))
    return render_template('question_admin.html', title='admin', form=form, questions=questions, dyear=year, dsemester=semester, dclas_id=clas_id, Q=Question)

@app.route('/finish_course/<int:student_id>/<int:course_id>')
@login_required
def finish_course(student_id, course_id):
    user_class = UserClas.query.filter_by(user_id=student_id, clas_id=course_id).first()
